...
def monitor_warning():
    ...
    recent_user_entries = AbnormalUser.objects.filter(time__range=(start_time, end_time))
    ...
    for entry in recent_host_entries:
        ...
        warning_result['time'] = entry.time
        warning_result['detail'] = '异常主机行为：'+entry.errprint
        warning_result['status'] = False
        warning_results.append(warning_result)
...
def send_email_view(warning_results):
    subject = settings.EMAIL_SUBJECT_PREFIX+' 态势告警'
    message = warning_results
